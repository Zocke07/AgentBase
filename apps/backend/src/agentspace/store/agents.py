...
class DuplicateAgentNameError(AgentValidationError):
    """`agent_defs.name` is UNIQUE (§4), and has to be: it is the handle a
    supervisor spawns by."""

    def __init__(self, name: str) -> None:
        super().__init__(f"An agent named {name!r} already exists.", field="name")
        self.name = name
...
_SELECT: Final[str] = (
    "SELECT id, name, role, system_prompt, provider, model, allowed_tools,"
...
class AgentDefStore:
    ...
    async def list_all(self) -> list[AgentDef]:
        return await asyncio.to_thread(self._list_sync, only_enabled=False)

    async def list_enabled(self) -> list[AgentDef]:
        """The roster a run is offered.
        ...
        for a built-in the user cannot delete.
        """
        return await asyncio.to_thread(self._list_sync, only_enabled=True)

    def _list_sync(self, *, only_enabled: bool) -> list[AgentDef]:
        clause = " WHERE enabled = 1" if only_enabled else ""
        with self._db.read() as connection:
            rows = connection.execute(f"{_SELECT}{clause} ORDER BY name").fetchall()
    ...
    def _create_sync(self, fields: dict[str, Any], cap: int) -> AgentDef:
        ...
        with self._db.write() as connection:
            if _name_taken(connection, name, excluding=None):
                raise DuplicateAgentNameError(name)
            connection.execute(
                "INSERT INTO agent_defs (id, name, role, system_prompt, provider,"
                ...
                (
                    ...
                    steps,
                    json.dumps([str(level) for level in approve]),
                    int(bool(fields.get("enabled", True))),
                    now,
                    now,
                ),
            )
    ...
    def _update_sync(self, definition_id: str, changes: dict[str, Any], cap: int) -> AgentDef:
        ...
        with self._db.write() as connection:
            existing = connection.execute(
                "SELECT id FROM agent_defs WHERE id = ?", (definition_id,)
            ).fetchone()
            if existing is None:
                raise AgentNotFoundError(definition_id)

            if "name" in columns and _name_taken(
                connection, columns["name"], excluding=definition_id
            ):
                raise DuplicateAgentNameError(columns["name"])
...
def _name_taken(connection: sqlite3.Connection, name: str, *, excluding: str | None) -> bool:
    """Check uniqueness inside the writing transaction.
    ...
    """
    row = connection.execute("SELECT id FROM agent_defs WHERE name = ?", (name,)).fetchone()
