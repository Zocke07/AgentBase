...
from agentbase.tools.sandbox import Sandbox, SandboxViolationError
...
class KnowledgeStore:
    ...
    async def write_note(self, space_id: str, path: str, content: str) -> KnowledgeNote:
        ...
        def write() -> None:
            ...
            temporary = resolved.with_suffix(resolved.suffix + ".agentbase-tmp")
    ...
    async def delete_folder(self, space_id: str, path: str) -> KnowledgeFolderDeleteResult:
        ...
        every file it holds under `.agentbase/backups/`, the way a move or a
    ...
    async def _resolve_file(self, space_id: str, path: str) -> tuple[Path, Path, str]:
        ...
        if any(part.startswith(".") for part in relative.parts):
            raise KnowledgePathError(
                "Hidden folders, including .obsidian and .agentbase, are managed by the "
...
def _atomic_write(path: Path, content: str) -> None:
    ...
    temporary = path.with_suffix(path.suffix + ".agentbase-tmp")
    temporary.write_text(content, encoding="utf-8")
...
def _backup_files(root: Path, files: Iterable[Path], label: str) -> Path:
    ...
    backup = root / ".agentbase" / "backups" / f"{stamp}-{label}-{uuid.uuid4().hex[:8]}"
